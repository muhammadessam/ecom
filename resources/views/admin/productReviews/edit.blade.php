    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-reviews.update", [$productReview->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('good') ? 'is-invalid' : '' }}" name="good" id="good">{{ old('good', $productReview->good) }}</textarea>
                @if($errors->has('good'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('bad') ? 'is-invalid' : '' }}" name="bad" id="bad">{{ old('bad', $productReview->bad) }}</textarea>
                @if($errors->has('bad'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('review') ? 'is-invalid' : '' }}" name="review" id="review">{{ old('review', $productReview->review) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" type="number" name="rate" id="rate" value="{{ old('rate', $productReview->rate) }}" step="1">
                @if($errors->has('rate'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ ($productReview->product ? $productReview->product->id : old('product_id')) == $id ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
