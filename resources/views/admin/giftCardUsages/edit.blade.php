...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.giftCardUsage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gift-card-usages.update", [$giftCardUsage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('giftcard') ? 'is-invalid' : '' }}" name="giftcard_id" id="giftcard_id" required>
                    @foreach($giftcards as $id => $giftcard)
                        <option value="{{ $id }}" {{ ($giftCardUsage->giftcard ? $giftCardUsage->giftcard->id : old('giftcard_id')) == $id ? 'selected' : '' }}>{{ $giftcard }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id" required>
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ ($giftCardUsage->customer ? $giftCardUsage->customer->id : old('customer_id')) == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
