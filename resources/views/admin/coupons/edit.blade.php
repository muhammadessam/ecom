    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.coupons.update", [$coupon->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $coupon->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Coupon::TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', $coupon->type) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_free_shipping') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_free_shipping" id="is_free_shipping" value="1" {{ $coupon->is_free_shipping || old('is_free_shipping', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_cat') ? 'is-invalid' : '' }}" name="product_cat_id" id="product_cat_id">
                    @foreach($product_cats as $id => $product_cat)
                        <option value="{{ $id }}" {{ ($coupon->product_cat ? $coupon->product_cat->id : old('product_cat_id')) == $id ? 'selected' : '' }}>{{ $product_cat }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_usage') ? 'is-invalid' : '' }}" type="number" name="total_usage" id="total_usage" value="{{ old('total_usage', $coupon->total_usage) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('usage_per_user') ? 'is-invalid' : '' }}" type="number" name="usage_per_user" id="usage_per_user" value="{{ old('usage_per_user', $coupon->usage_per_user) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $coupon->is_active || old('is_active', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text" name="start" id="start" value="{{ old('start', $coupon->start) }}">
                @if($errors->has('start'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text" name="end" id="end" value="{{ old('end', $coupon->end) }}">
                @if($errors->has('end'))
