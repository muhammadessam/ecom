...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.cmsPage.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cms-pages.update", [$cmsPage->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('question') ? 'is-invalid' : '' }}" type="text" name="question" id="question" value="{{ old('question', $cmsPage->question) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('answer') ? 'is-invalid' : '' }}" type="text" name="answer" id="answer" value="{{ old('answer', $cmsPage->answer) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cat') ? 'is-invalid' : '' }}" name="cat_id" id="cat_id">
                    @foreach($cats as $id => $cat)
                        <option value="{{ $id }}" {{ ($cmsPage->cat ? $cmsPage->cat->id : old('cat_id')) == $id ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
                @if($errors->has('cat'))
